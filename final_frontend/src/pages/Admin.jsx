...
export default function Admin() {
  ...
  function openEditBook(book) {
    ...
    setSelectedFile(null);
    setImagePreview(book.coverImageUrl ? getImageUrl(book.coverImageUrl) : null);
    setShowBookForm(true);
  }
  ...
  async function uploadBookImage(bookId) {
    if (!selectedFile) return;
    const formData = new FormData();
    formData.append("file", selectedFile);
    await API.post(`/books/upload-image/${bookId}`, formData);
  }
  ...
  async function handleSaveBook(e) {
    ...
    try {
      ...
      // Clean payload to ONLY include what the backend BookRequest DTO expects
      const payload = {
      ...
      if (selectedFile) {
        toast.loading("Uploading cover image...", { id: "uploading" });
        await uploadBookImage(bookId);
  ...
  async function handleDeleteBook(bookId, title) {
    ...
    try { await API.delete(`/books/${bookId}`); toast.success("Book deleted"); loadBooks(); }
    catch { toast.error("Failed to delete book"); }
  }

  async function handleToggleFeatured(bookId) {
    try { await API.patch(`/books/${bookId}/featured`); toast.success("Featured updated"); loadBooks(); }
  ...
  async function handleUpdateStock() {
    ...
    try {
      setSavingStock(true);
      await API.patch(`/books/${stockBook.bookId}/stock`, null, { params: { quantity: parseInt(stockQty) } });
            ...
            <form onSubmit={handleSaveBook} className="book-form">
              ...
              <div className="form-group full-width">
                ...
                <div className="book-image-upload">
                  ...
                  <div className="book-image-input-wrap">
                    <input type="file" accept="image/*" onChange={handleFileChange} id="cover-upload" hidden />
                    <label htmlFor="cover-upload" className="btn-ghost">
                      {imagePreview ? "Change Image" : "Select Cover Image"}
                    </label>
                    <p className="form-hint">Recommended: 800x1200px (WebP/JPG/PNG)</p>
                  </div>
                </div>
              </div>
              <div className="form-group full-width">
